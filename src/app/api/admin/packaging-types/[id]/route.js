...
export async function GET(req, { params }) {
  ...
  const item = await PackagingType.findById(id).lean();
  if (!item) {
...
export async function PATCH(req, { params }) {
  ...
  try {
    ...
    if (update.code) {
      const duplicate = await PackagingType.findOne({
        code: update.code,
        _id: { $ne: id },
      }).select("_id");
    ...
    const updated = await PackagingType.findByIdAndUpdate(
      id,
      { $set: update },
      { new: true }
    ).lean();
    ...
    return NextResponse.json(
      { success: false, message: "خطا در ویرایش نوع بسته‌بندی" },
      { status: 500 }
    );
  }
}
...
export async function DELETE(req, { params }) {
  ...
  await PackagingType.findByIdAndDelete(id);
